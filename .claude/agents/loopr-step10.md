...
Pydantic v2 with model_config = ConfigDict(extra="forbid") on every BaseModel.


mypy --strict clean; type hints on every signature.


Deterministic rules engines anchor any LLM extraction/generation -- the LLM never has the last
word on anything that must be correct. Specify the deterministic layer explicitly.


LLM routing is Google-native (Gemini/Vertex) unless the PRD says otherwise; pin exact model strings.
