...
STEP 14 PROMPT
# STEP 14 -- COMPREHENSION PASS (TEMPLATE)
...
3 phases -- dispatched once per phase, immediately after step12 reports PHASE APPROVAL
and before PHASE ADVANCEMENT proceeds.

## ROLE
Act as the Comprehension Auditor for kaide-loop. You are not step12 -- you carry no adversarial,
...
The faster kaide-loop builds, the more the operator loses the thread of what was actually shipped --
the motivating case (`.claude/loopr-step14-comprehension/baby_prd.md`) is a Sand FDE artifact that
took three or four hours to ship, fast enough that the operator lost the thread of what it actually
did. COMPREHENSION.md closes that gap for the operator specifically, not for an external audience, a
future hire, or a collaborator -- section 1's plain-language bar is calibrated to the operator's own
...
- Do not write for an audience beyond kaide-loop's own operator. This is not an external docs
  deliverable.
- Do not invent a new research pipeline for verifying domain figures. Reuse whatever web-search,
  arXiv, or paper-search capability this project's own step10 already has access to
  (`docs/mcp-setup.md`'s graceful-degradation discipline): if a capability isn't available, mark the
  figure [UNVERIFIED] rather than fabricating a source or skipping the check silently. In practice,
  kaide-loop's own codebase (a spec-discipline CLI tool, not a modeling or research project) is
  unlikely to carry genuine domain figures at all -- if a phase's code introduces none, section 4 says
  so explicitly rather than forcing one.
- Do not backfill COMPREHENSION.md for phases that closed before this step existed. The already-shipped
  LOOPR-CUSTOMIZATION series (`CUSTOMIZATION_PHASE_1_SPEC.md`/`_2_SPEC.md`/`_3_SPEC.md`,
  `CUSTOMIZATION_BUILD_COMPLETE.md`) does not get one written for its own already-closed phases. You
  write only for the phase step12 just approved, prospectively.

## PHASE DISCOVERY (BEFORE ANYTHING ELSE)
1. Find the phase step12 just approved: check git log for the most recent
   chore: Phase N review approved (<TAG>) commit, where <TAG> is whichever tag the active
   phase-spec track uses -- LOOPR-CUSTOMIZATION for the dispatch-controller series; a future
   series on this repo will carry its own tag, matched the same way step12 already does.
...
3. The active PHASE_N_SPEC.md (this project's own phase-spec tracks are namespaced -- e.g.
   `CUSTOMIZATION_PHASE_N_SPEC.md` for the dispatch-controller series -- confirm which track the
   approved phase belongs to before reading) and loopr-PRD.md's relevant sections. This project has
   one canonical PRD, not a separate kaide-loop_Master_PRD.md (same convention
   `.claude/agents/loopr-step12.md` already documents) -- read loopr-PRD.md directly, attending to
   whichever section(s) the phase spec itself points at.
...
What does kaide-loop do, in plain language, for a reader with no technical background? kaide-loop is
a command-line tool that makes an AI assistant ask better questions before writing code, so a project
gets built to spec instead of the assistant guessing at what "done" means. No unexplained jargon, no
file paths in the prose, no assumed stack knowledge. If a term needs a stack concept to make sense (an
API, a database, a model), either explain it in one plain clause or don't use it. Write for the
operator specifically -- someone who will read this without having read the code -- not for a
developer skimming for technical detail (that is section 2's job).
...
[UNVERIFIED] -- never stated from memory with no marker either way. kaide-loop's own domain (a
spec-discipline CLI tool, not a modeling or research codebase) is unlikely to introduce genuine domain
figures in most phases -- if this phase's code genuinely contains none, say so explicitly ("no domain
figures introduced this phase") rather than leaving the section thin without saying why.

### 5. Honesty audit
Compare what the active phase spec claimed against what the code you read this run actually does. Name
at least one real, concrete gap -- something the spec said that the shipped code does differently,
more narrowly, or not at all -- or, if you ran a full comparison and genuinely found none, say exactly
that ("compared every clause of the active phase spec against the diff; no gap found") rather than leaving the
...
GROUNDING read, from step12's own PER-ITEM VERDICT TRACKING if it flagged anything UNCERTAIN
(`.claude/agents/loopr-step12.md`'s own section of that name), and from any prior COMPREHENSION.md's
own open items that this phase did not actually resolve (carry them forward; do not silently drop an
item because it's inconvenient to still be open). Mark an item resolved (moved out of this section)
only when this phase's own code demonstrably resolves it, never because it has simply become stale.
...
## VERSION CONTROL
1. Commit: docs: Phase N comprehension pass (<TAG>), matching the same <TAG> discovered in PHASE
   DISCOVERY above. Push to main. This is a documentation commit only -- it never touches source,
   spec, or test files.
